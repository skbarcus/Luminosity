...
void BCM_Charges_Percent(Int_t run){
 ...
 //Double_t I, u1c_a, rclk_a;
  ...
  for(Int_t i=1;i<nevts;i++)
      ...
      if(thr_u1[i]==1 && (thr_u1[i+1]==0 || i+1==nevts) )
	{
	  ...
	  if(i!=nevts-1)
	    {
	      u1c_beam = u1c_beam + u1c_a[i+1] - u1c_a[rising_edge_u1];	  
	      rclk_beam_u1 = rclk_beam_u1 + rclk_a[i+1] - rclk_a[rising_edge_u1];
	    }
	  else
	    {
	      u1c_beam = u1c_beam + u1c_a[i] - u1c_a[rising_edge_u1];	  
  ...
  for(Int_t i=1;i<nevts;i++)
      ...
      if(thr_d1[i]==1 && (thr_d1[i+1]==0 || i+1==nevts) )
	{
	  ...
	  if(i!=nevts-1)
	    {
	      d1c_beam = d1c_beam + d1c_a[i+1] - d1c_a[rising_edge_d1];	  
	      rclk_beam_d1 = rclk_beam_d1 + rclk_a[i+1] - rclk_a[rising_edge_d1];
	    }
	  else
	    {
	      d1c_beam = d1c_beam + d1c_a[i] - d1c_a[rising_edge_d1];	  
	      rclk_beam_d1 = rclk_beam_d1 + rclk_a[i] - rclk_a[rising_edge_d1];	     
	    }
